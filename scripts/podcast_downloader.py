...
# Bounded (max-depth, no-recursion) structural search helpers for NordVPN's
# window. These exist because NordVPN's SwiftUI layout has no stable AX
# identifiers and its exact sibling-index layout (how many children a given
# group has, and in what order) is NOT guaranteed to be identical across
# NordVPN app versions, window sizes, or macOS versions on a different Mac —
# a promo banner being dismissed, a slightly different build, or a resized
# window can silently shift a fixed "item N of ..." path onto the wrong
# element. Instead of trusting a hardcoded position, these search the tree
# (bounded to maxDepth, same iterative stack-based walk as findButtonByName
# above — no recursion, so it can't blow the AppleScript stack) for an
# element matching a distinctive, version-independent signature.
...
class VPNController:
    ...
    def _poll_verify(
        ...
        L2 is the minimum condition for "connected" — UNLESS the provider reuses
        the same local tunnel gateway address for every server. NordVPN does
        exactly this (confirmed empirically: it's always 10.5.0.2 regardless of
        which city is connected), so switching directly from one already-
        connected server to another produces NO visible route change at all,
        even though the new server is genuinely live — the L2 check alone would
        then wait out the entire verify_timeout on every such transition. To
        avoid that, a public IP that has already moved away from baseline is
        also accepted on its own (L1+L3, route-static bypass) — a different
        real IP is itself strong, independent evidence of a new connection.
        L3/L4 are otherwise optional and never cause a failure if APIs are
        rate-limited.
        """
        ...
        while time.monotonic() < deadline:
            ...
            if not route_changed:
                # Route-static bypass: before accepting "still pending", check
                # whether the public IP has already moved away from baseline —
                # see the docstring note on providers (NordVPN) that reuse the
                # same local tunnel gateway for every server. If require_provider
                # _in_org demands a country match and it's wrong, fall through to
                # the normal pending/retry path below rather than accepting.
                ip_info = self.net.public_ip_info()
                ip = (ip_info or {}).get("ip")
                if ip and baseline_ip and ip != baseline_ip:
                    info = {
                        "ip": ip,
                        "country": (ip_info or {}).get("country"),
                        "org": (ip_info or {}).get("org"),
                    }
                    country_ok = self._is_connected_to(
                        info, target_cc, require_provider_in_org, provider_token
                    )
                    if country_ok or not require_provider_in_org:
                        verify_level = "tunnel+ip_route_static" + ("+country" if country_ok else "")
                        country_check_bypass = "verified" if country_ok else "wrong"
                        self._record_ip(ip)
                        self.state.data["vpn_verify_level"] = verify_level
                        self.state.save()
                        self.logger.log(
                            f"VPN connected (route-static bypass): level={verify_level} "
                            f"tunnels={tunnels} route={current_route} ip={ip} "
                            f"country_check={country_check_bypass} after {attempts}s",
                            step="06", status="connected_verified",
                            verify_level=verify_level, tunnels=tunnels, ip=ip, attempts=attempts,
                        )
